...
On host activation the plugin fetches `dsh-opc-assets.tar.gz` from the latest
GitHub release into `~/.dsh/dsh-opc/assets`. The release asset contains the
versioned character manifest and WebM clips. Clips are intentionally ignored
by Git; use `pnpm assets:dummy` for local placeholder clips, replace them with
licensed art before release, then run `pnpm assets:pack`.
