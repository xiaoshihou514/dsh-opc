...
fn ledger_path() -> PathBuf {
    let home = env::var("DSH_OPC_PET_HOME")
        .ok()
    ...
    PathBuf::from(home)
        .join(".dsh")
        .join("dsh-opc-pet")
        .join("notification-ledger.json")
}
fn load_ledger(path: &PathBuf) -> BTreeSet<String> {
    fs::read_to_string(path)
        .ok()
        .and_then(|text| serde_json::from_str(&text).ok())
        .unwrap_or_default()
}
fn save_ledger(path: &PathBuf, entries: &BTreeSet<String>) {
...
fn loopback_url(value: &str) -> Result<String, String> {
    ...
    {
        return Err("DSH URL must be an HTTP(S) loopback address".into());
    }
...
async fn fetch(state: &PetState) -> Result<Value, String> {
    reqwest::Client::new()
        .get(format!("{}/dsh-opc/v1/state", state.url))
...
async fn fetch_manifest(state: &PetState) -> Result<Value, String> {
    reqwest::Client::new()
        .get(format!("{}/dsh-opc/v1/assets/manifest.json", state.url))
        .timeout(Duration::from_secs(5))
        .send()
        .await
        .map_err(|e| e.to_string())?
...
fn alert_key(session: &Value, now: i64) -> Option<(String, String)> {
    ...
    match session.get("state")?.as_str()? {
        "await" => Some((
            format!(
                "{id}:permission:{}",
                session.get("approval")?.get("id")?.as_str()?
            ),
            "DSH 需要你的授权".into(),
        )),
        "error" => Some((
            format!("{id}:error:{}", session.get("error")?.get("id")?.as_str()?),
            "DSH 会话发生错误".into(),
        )),
        _ => {
            let since = session.get("runningSince")?.as_i64()?;
            let elapsed = now - since;
            let milestone = if elapsed >= 60 * 60_000 {
                Some((elapsed / 3_600_000) * 60)
            } else {
                [5, 10, 20, 30, 45]
                    .into_iter()
                    .filter(|minutes| elapsed >= minutes * 60_000)
                    .last()
            }?;
            Some((
                format!("{id}:running:{milestone}"),
                format!("DSH 会话已运行 {milestone} 分钟"),
            ))
        }
    }
}
            ...
            if let Some(sessions) = snapshot.get("sessions").and_then(Value::as_array) {
                for session in sessions {
                    if let Some((key, title)) = alert_key(session, now) {
                        ...
                        if seen.insert(key) {
                            save_ledger(&state.ledger_path, &seen);
                            let _ = app
                                .notification()
                                .builder()
                                .title(title)
...
#[tauri::command]
async fn snapshot(state: State<'_, PetState>) -> Result<Value, String> {
    fetch(&state).await
}
#[tauri::command]
async fn manifest(state: State<'_, PetState>) -> Result<Value, String> {
    fetch_manifest(&state).await
}
#[tauri::command]
