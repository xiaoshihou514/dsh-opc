...
function Worker({
  ...
  return (
    ...
    >
      ...
      <div
        className="opc-animationToggle"
        role="button"
        tabIndex={0}
        aria-label={collapsed ? "展开角色动画" : "收缩角色动画"}
        title={collapsed ? "展开角色动画" : "收缩角色动画"}
        onClick={(event) => {
      ...
      >
        {collapsed ? (
          <span
            className={`opc-pokeball${running ? " opc-pokeball-running" : ""}`}
            aria-hidden="true"
          />
        ) : failed ? (
          <span className="opc-fallback" aria-hidden="true">
            ◉
          </span>
        ) : (
          <LoopVideo
