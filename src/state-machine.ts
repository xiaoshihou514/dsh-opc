import type { ApprovalView, ErrorView, SessionState, SessionView } from './protocol.ts'
...
const READ_TOOLS = new Set(['read', 'glob', 'grep', 'search', 'webfetch', 'browse', 'fetch', 'list', 'ls'])
...
export function project(facts: SessionFacts): SessionView {
  ...
  return {
    id: facts.id, title: facts.title, model: facts.model, character: facts.character,
    state, stateSince: facts.stateSince,
    ...(facts.workspace === undefined ? {} : { workspace: facts.workspace }),
    ...(facts.runningSince === undefined ? {} : { runningSince: facts.runningSince }),
